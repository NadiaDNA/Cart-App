...
export default function CartItem({ product }) {
  ...
  const subTotal = (product.price * quantity).toFixed(2);
    ...
    return (
     <div className="bg-pink-100 border p-4 rounded-lg shadow-md flex flex-col sm:flex-row items-center justify-between gap-6 hover:shadow-lg transition-shadow shadow-gray-300 duration-200">
        <div className="flex items-center gap-4 w-full sm:w-auto">
            <img src={product.image} alt={product.title} className="h-20 w-20 object-contain bg-pink-100 border rounded-2xl p-2 hover:shadow-lg transition-shadow shadow-[#701A42] duration-200" />
                <div className="flex flex-col gap-1">
                    <h3 className="font-semibold text-sm text-gray-800 line-clamp-2 leading-tight min-h-[40px]">{product.title}</h3>
                    <div className="flex flex-col gap-0">
                        <p className="text-sm text-gray-500">Stock: {product.rating?.count || 0} </p>
